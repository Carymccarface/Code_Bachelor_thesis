...
data = pd.read_csv("all_features_pp_spin_fixed.csv")
print('----    Done!    ----')
...
data_train_x, data_test_x, data_train_y, data_test_y = train_test_split(
    data.drop(columns=['tot_mag_mom', 'E_formation','mag_order','encoded']), # Remove both outputs encoded
    data['encoded'], # Choose which output is target mag_mom/energy encoded
    test_size=0.25,
    random_state=2
)
print('----    Done!    ----')
...
if 1 == 1: # Set 1 if training RF
    ...
    if 1 == 0: # Set 1 if Grid Searching
        ...
        rf_mse = accuracy_score(data_test_y, rf_y_pred)
        rf_r2 = f1_score(data_test_y, rf_y_pred)

        print(f"Random Forest Test Accuracy: {rf_mse:.3f}, F1: {rf_r2:.3f}")
...
if 1 == 0: 
    ...
    if 1 == 0: # Set 1 if gridsearching
        ...
        xgb_clf = XGBClassifier(random_state=1, verbosity=0, tree_method='hist')
        ...
        xgb_mse = accuracy_score(data_test_y, xgb_y_pred)
        xgb_r2 = f1_score(data_test_y, xgb_y_pred)

        print(f"XGBoost Test Accuracy: {xgb_mse:.3f}, F1: {xgb_r2:.3f}")


    else: # Use known best parameters
        # Create and train the model
        xgb_model = XGBClassifier(**best_params_energy)
        ...
        # Evaluate
        xgb_mse = accuracy_score(data_test_y, y_pred)
        xgb_r2 = f1_score(data_test_y, y_pred)
